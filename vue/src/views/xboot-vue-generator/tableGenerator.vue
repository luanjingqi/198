  ...
  <div class="search">
      ...
      <Row>
        <Table border :columns="columns" :data="data" ref="table">
          ...
          <template slot-scope="{ row }" slot="defaultSort">
            <i-switch
              v-model="row.defaultSort"
              @on-change="changeDefaultSort(row, $event)"
              :disabled="(!row.sortable||defaultSort)&&!row.defaultSort||!row.sortable"
            >
              <span slot="open">开</span>
              <span slot="close">关</span>
            </i-switch>
          ...
          <template slot-scope="{ row }" slot="defaultSortType">
            <Select
              v-model="row.defaultSortType"
              @on-change="changeDefaultSortType(row, $event)"
    ...
    <Modal :title="modalTitle" v-model="modalVisible" :mask-closable="false" :width="750">
      <Form ref="form" :model="form" :label-width="100" :rules="formValidate" inline>
        ...
        <FormItem label="显示级别" prop="level" v-show="form.type=='area'">
          <Select v-model="form.level" style="width:345px">
        ...
        <FormItem label="显示级别" prop="searchLevel" v-show="form.searchType=='area'">
          <Select v-model="form.searchLevel" style="width:345px">
            <Option value="0">仅显示省</Option>
            <Option value="1">显示省和县</Option>
            <Option value="2">显示省市县</Option>
            <Option value="3">显示省市县街道</Option>
    ...
    <Modal title="根据实体类配置" v-model="entityModal" :width="700" :fullscreen="fullEntity">
      ...
      <div slot="footer">
        ...
        <Button type="warning" icon="md-play" @click="generateEntityData">读取实体类字段</Button>
